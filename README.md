...
#### **Windows**
1. Run the `install.bat` script. It will automatically create a virtual environment and install all dependencies.
   ```
   install.bat
   ```
...
#### **Local Models Installation (Optional)**

If you want to use local models, you may need to install additional dependencies:

1. Run the specialized installation script for llama-cpp-python:
   - Windows: `install_llama_cpp.bat`
   - Linux/macOS: `./install_llama_cpp.sh`

These scripts handle the complex build process for llama-cpp-python, which is required for local model support. See `LOCAL_MODELS_README.md` for detailed instructions.

---

## :eyes: Usage

Once installation and configuration are complete, you can run the application using the provided launcher scripts:
...
Alternatively, you can manually activate the virtual environment and run the application:
